...
// Única vía a la colección: entrega entidades Post o lanza PostsDataError.
...
export class PostsRepository {
  ...
  async getPosts(): Promise<Post[]> {
    ...
    try {
      ...
      throw new PostsDataError('architecture: no se pudieron leer los artículos de la colección');
    }
...
async function loadArchitectureEntries(): Promise<unknown[]> {
  const { getCollection } = await import('astro:content'); return getCollection('architecture');
}
...
function expectRelated(data: Record<string, unknown>, index: number): string[] | null {
  ...
  if (value === undefined) return null;
  if (!Array.isArray(value) || value.length === 0 || !value.every((item) => typeof item === 'string' && /^\/posts\/.+/.test(item))) throw new PostsDataError(`architecture: el artículo ${index} tiene un campo "related" que debe ser un arreglo de rutas internas /posts/<id>`);
...
function expectTags(data: Record<string, unknown>, index: number): string[] {
  if (!Array.isArray(data.tags) || !data.tags.every((tag) => typeof tag === 'string')) {
    throw new PostsDataError(`architecture: el artículo ${index} tiene un campo "tags" que debe ser un arreglo de texto`);
  }
